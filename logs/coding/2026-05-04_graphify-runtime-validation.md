...
## Work Summary (2026-05-04 local) - CI queue-runner fix

### Goal
- Fix PR #74 Routing Validators failure caused by another isolated validator missing the new `till-done.ts` helper dependency.

### Files Changed and Why
- `scripts/validate-queue-runner.sh`: copies, compiles, and loads `graphify-validation-decision.ts` alongside `till-done.ts` for the queue-runner validator because queue-runner depends on till-done.

### RED Evidence
- PR #74 gate failed Routing Validators.
- GitHub job log showed `scripts/validate-queue-runner.sh --skip-live` failed after earlier validators passed.
- Local `bash scripts/validate-core-workflows.sh --report /tmp/phase2-core.md --summary-json /tmp/phase2-core.json` had already revealed the same dependency class with `Cannot find module './graphify-validation-decision.ts'`.

### GREEN Evidence
- `bash scripts/validate-queue-runner.sh --skip-live --report /tmp/phase2-queue-runner.md --summary-json /tmp/phase2-queue-runner.json` passed with `Queue-runner validation PASS`.
- `bash scripts/check-repo-static.sh` passed with `repo-static-checks-ok`.
- `git diff --check` passed with no output.

### Wiring Verification
- Queue-runner validator now copies the helper, includes it in isolated TypeScript compile, and loads it in the optional live probe extension list.

### Behavior Changes and Risk Notes
- No runtime behavior change; validator packaging/wiring only.
...
- Already run: extension unit validator with 3 consecutive passes, foundation compile, Graphify discovery validator, core workflows validator, queue-runner validator, static checks, and diff whitespace check.
