...
write_json_summary() {
  ...
  # TS emitter reads the three parallel files (names/statuses/details),
  # validates the shape against the typed contract, and writes
  # byte-equivalent JSON to $SUMMARY_JSON_PATH. The previous inline
  # Python emission produced the same bytes (locked down by a golden
  # test in tests/extension-units/validator-report.test.ts).
  #
  ...
  if ! "$NODE_BIN" --import "$TSX_IMPORT" "$REPO_ROOT/scripts/lib/emit-validator-report.ts" \
    ...
    echo "harness-package-validation: FAIL (emit-validator-report failed; see stderr above)" >&2
    exit 1
  fi
}
...
package_template = json.loads((root / '.pi/agent/package/templates/package.template.json').read_text(encoding='utf-8'))
checks = {
    ...
    '.pi/agent/docs/harness_package_install.md': 'harness-package.ts bootstrap' in (root / '.pi/agent/docs/harness_package_install.md').read_text(encoding='utf-8') and 'harness:init-feature' in (root / '.pi/agent/docs/harness_package_install.md').read_text(encoding='utf-8') and 'harness:product-intake' in (root / '.pi/agent/docs/harness_package_install.md').read_text(encoding='utf-8') and 'harness:install' in (root / '.pi/agent/docs/harness_package_install.md').read_text(encoding='utf-8') and 'operator_manual.md' in (root / '.pi/agent/docs/harness_package_install.md').read_text(encoding='utf-8'),
    '.pi/agent/docs/operator_install_guide.md': 'harness:init-feature' in (root / '.pi/agent/docs/operator_install_guide.md').read_text(encoding='utf-8') and 'harness:product-intake' in (root / '.pi/agent/docs/operator_install_guide.md').read_text(encoding='utf-8') and 'harness:install' in (root / '.pi/agent/docs/operator_install_guide.md').read_text(encoding='utf-8') and '/skill:g-grill' in (root / '.pi/agent/docs/operator_install_guide.md').read_text(encoding='utf-8'),
    ...
    'package.json': 'validate:harness-package' in package_json.get('scripts', {}) and 'harness:init-feature' in package_json.get('scripts', {}) and 'harness:product-intake' in package_json.get('scripts', {}) and 'harness:install' in package_json.get('scripts', {}),
    'package.template.json': 'harness:init-feature' in package_template.get('scripts', {}) and 'harness:product-intake' in package_template.get('scripts', {}) and 'harness:install' in package_template.get('scripts', {}),
}
...
PY
  ...
  if "$PYTHON_BIN" "$TMP_ROOT/check_3_manifest_and_install_docs.py" "$REPO_ROOT" >"$out" 2>&1; then
    local detail="package manifest, initiative-helper script wiring, install docs, package-script aliases, and operator-doc entrypoints are present for repeatable harness bootstrap and onboarding."
    record_result "$name" "PASS" "$detail"
    append_summary_row "$name" "PASS" "$detail"
    append_check_section "$name" "PASS" "$cmd" "- output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  else
