...
write_json_summary() {
  ...
  if ! "$NODE_BIN" --import "$TSX_IMPORT" "$REPO_ROOT/scripts/lib/emit-validator-report.ts" \
    ...
    echo "core-workflows-validation: FAIL (emit-validator-report failed; see stderr above)" >&2
    exit 1
  fi
}
...
JSON
  ...
  cp "$REPO_ROOT/.pi/agent/extensions/lib/"* "$workdir/.pi/agent/extensions/lib/"
  ...
  cp "$REPO_ROOT/scripts"/harness-*.ts "$workdir/scripts/"
