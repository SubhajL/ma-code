#!/usr/bin/env bash
set -u -o pipefail
IFS=$'\n\t'

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
DATE_STAMP="$(date +%F)"
REPORT_DIR="$REPO_ROOT/reports/validation"
DEFAULT_REPORT="$REPORT_DIR/${DATE_STAMP}_queue-semantics-validation-script.md"
DEFAULT_SUMMARY_JSON="$REPORT_DIR/${DATE_STAMP}_queue-semantics-validation-script.json"
PYTHON_BIN="${PYTHON_BIN:-python3}"
NODE_BIN="${NODE_BIN:-node}"
TSX_IMPORT="${TSX_IMPORT:-tsx}"
REPORT_PATH="$DEFAULT_REPORT"
SUMMARY_JSON_PATH="$DEFAULT_SUMMARY_JSON"
KEEP_TEMP=0
TMP_ROOT=""
CHECK_NAMES=()
CHECK_STATUS=()
CHECK_DETAILS=()
FAILED_CHECKS=0

usage() {
  cat <<EOF
Usage: $(basename "$0") [options]

Options:
  --report <path>          Write markdown report to a custom path
  --summary-json <path>    Write JSON summary to a custom path
  --keep-temp              Keep temporary validation files
  -h, --help               Show this help text

Environment overrides:
  PYTHON_BIN=<path>        Python executable to use (default: python3)
EOF
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --report)
      REPORT_PATH="$2"
      shift 2
      ;;
    --summary-json)
      SUMMARY_JSON_PATH="$2"
      shift 2
      ;;
    --keep-temp)
      KEEP_TEMP=1
      shift
      ;;
    -h|--help)
      usage
      exit 0
      ;;
    *)
      echo "Unknown option: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
done

mkdir -p "$REPORT_DIR" "$(dirname "$REPORT_PATH")" "$(dirname "$SUMMARY_JSON_PATH")"
TMP_ROOT="$(mktemp -d)"
SUMMARY_TABLE_FILE="$TMP_ROOT/summary_table.md"
DETAILS_FILE="$TMP_ROOT/detail_sections.md"
: > "$SUMMARY_TABLE_FILE"
: > "$DETAILS_FILE"

cleanup() {
  local exit_code=$?
  if [[ $KEEP_TEMP -eq 0 && -n "$TMP_ROOT" && -d "$TMP_ROOT" ]]; then
    rm -rf "$TMP_ROOT"
  else
    echo "Temporary validation files kept at: $TMP_ROOT" >&2
  fi
  exit "$exit_code"
}
trap cleanup EXIT

record_result() {
  local name="$1"
  local status="$2"
  local detail="$3"

  CHECK_NAMES+=("$name")
  CHECK_STATUS+=("$status")
  CHECK_DETAILS+=("$detail")

  if [[ "$status" == "FAIL" ]]; then
    FAILED_CHECKS=$((FAILED_CHECKS + 1))
  fi
}

append_summary_row() {
  local name="$1"
  local status="$2"
  local detail="$3"
  printf '| %s | %s | %s |\n' "$name" "$status" "${detail//$'\n'/ <br> }" >> "$SUMMARY_TABLE_FILE"
}

append_check_section() {
  local name="$1"
  local status="$2"
  local command="$3"
  local evidence="$4"

  {
    printf '\n## %s\n' "$name"
    printf -- '- Status: %s\n\n' "$status"
    printf '### Command\n```bash\n%s\n```\n\n' "$command"
    printf '### Key Evidence\n%b\n' "$evidence"
  } >> "$DETAILS_FILE"
}

write_header() {
  cat > "$REPORT_PATH" <<EOF
# Automated Validation Report — Queue Semantics

- Date: $DATE_STAMP
- Generated at: $(date '+%Y-%m-%dT%H:%M:%S%z')
- Repo root: $REPO_ROOT
- Python binary: $PYTHON_BIN
- Temporary root: $TMP_ROOT

## Summary Table

| Check | Status | Notes |
|---|---|---|
EOF
}

write_json_summary() {
  local names_file="$TMP_ROOT/summary_names.txt"
  local statuses_file="$TMP_ROOT/summary_status.txt"
  local details_file="$TMP_ROOT/summary_details.txt"
  : > "$names_file"
  : > "$statuses_file"
  : > "$details_file"

  local i
  for i in "${!CHECK_NAMES[@]}"; do
    printf '%s\n' "${CHECK_NAMES[$i]}" >> "$names_file"
    printf '%s\n' "${CHECK_STATUS[$i]}" >> "$statuses_file"
    printf '%s\n' "${CHECK_DETAILS[$i]}" >> "$details_file"
  done

  # Emit the canonical JSON via the typed contract module
  # (`.pi/agent/extensions/lib/validator-report.ts`, ADR-0007). The
  # TS emitter validates the shape against the typed contract and
  # writes byte-equivalent JSON to $SUMMARY_JSON_PATH. Byte-
  # equivalence with the prior Python emission is locked down by a
  # golden test in tests/extension-units/validator-report.test.ts.
  #
  # The script runs under `set -u -o pipefail` (no -e), so a non-zero
  # exit from the emitter would otherwise be silently dropped on the
  # floor — and downstream consumers would see a missing JSON file
  # instead of a typed contract violation. Surface the failure
  # explicitly here.
  if ! "$NODE_BIN" --import "$TSX_IMPORT" "$REPO_ROOT/scripts/lib/emit-validator-report.ts" \
      --out "$SUMMARY_JSON_PATH" \
      --names-file "$names_file" \
      --statuses-file "$statuses_file" \
      --details-file "$details_file"; then
    echo "Queue-semantics validation FAIL (emit-validator-report failed; see stderr above)" >&2
    exit 1
  fi
}

check_1_queue_schema_contract() {
  local name="1. queue schema defines versioned queue object"
  local out="$TMP_ROOT/check_1_queue_schema_contract.txt"
  local cmd="$PYTHON_BIN $TMP_ROOT/check_1_queue_schema_contract.py"

  cat > "$TMP_ROOT/check_1_queue_schema_contract.py" <<'PY'
import json, sys
from pathlib import Path
root = Path(sys.argv[1])
schema = json.loads((root / '.pi/agent/state/schemas/queue.schema.json').read_text(encoding='utf-8'))
assert schema['type'] == 'object', f"expected top-level object, found {schema['type']!r}"
required = set(schema['required'])
expected = {'version', 'paused', 'activeJobId', 'jobs'}
assert expected.issubset(required), f"missing required top-level fields: {sorted(expected - required)}"
props = schema['properties']
assert props['version']['const'] == 1, 'version must be const 1'
assert props['paused']['type'] == 'boolean', 'paused must be boolean'
assert props['activeJobId']['type'] == ['string', 'null'], 'activeJobId must allow string or null'
assert props['jobs']['type'] == 'array', 'jobs must be an array'
items = props['jobs']['items']
job_required = set(items['required'])
expected_job_required = {'id', 'goal', 'priority', 'status'}
assert expected_job_required.issubset(job_required), f"missing required job fields: {sorted(expected_job_required - job_required)}"
status_enum = set(items['properties']['status']['enum'])
assert {'queued', 'running', 'blocked', 'done', 'failed'} == status_enum, 'job status enum mismatch'
priority_enum = set(items['properties']['priority']['enum'])
assert {'low', 'medium', 'high'} == priority_enum, 'job priority enum mismatch'
quality_input = items['properties'].get('qualityInput')
assert quality_input is not None, 'qualityInput schema entry is required for structured queue→quality runtime input'
assert quality_input['type'] == ['object', 'null'], 'qualityInput must allow object or null'
assert set(quality_input['required']) == {'sourcePacketId', 'sourceHandoff'}, 'qualityInput required fields mismatch'
assert quality_input['properties']['sourceHandoff']['$ref'] == './handoff.schema.json', 'qualityInput.sourceHandoff must reference the structured handoff schema'
graphify_input = items['properties'].get('graphifyOrchestration')
assert graphify_input is not None, 'graphifyOrchestration schema entry is required for explicit research Graphify queue-session input'
assert graphify_input['type'] == ['object', 'null'], 'graphifyOrchestration must allow object or null'
assert set(graphify_input['required']) == {'enabled', 'need'}, 'graphifyOrchestration required fields mismatch'
assert 'curated_research' in graphify_input['properties']['purpose']['enum'], 'graphifyOrchestration purpose must support curated_research'
print('queue-schema-ok')
PY

  if "$PYTHON_BIN" "$TMP_ROOT/check_1_queue_schema_contract.py" "$REPO_ROOT" >"$out" 2>&1; then
    local detail="Queue schema uses the expected versioned top-level object with bounded job fields, structured quality input, and explicit research Graphify orchestration input."
    record_result "$name" "PASS" "$detail"
    append_summary_row "$name" "PASS" "$detail"
    append_check_section "$name" "PASS" "$cmd" "- schema output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  else
    local detail="Queue schema still does not match the intended versioned queue object contract."
    record_result "$name" "FAIL" "$detail"
    append_summary_row "$name" "FAIL" "$detail"
    append_check_section "$name" "FAIL" "$cmd" "- output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  fi
}

check_2_runtime_placeholder_alignment() {
  local name="2. queue runtime template matches finalized empty-state shape"
  local out="$TMP_ROOT/check_2_runtime_placeholder_alignment.txt"
  local cmd="$PYTHON_BIN $TMP_ROOT/check_2_runtime_placeholder_alignment.py"

  cat > "$TMP_ROOT/check_2_runtime_placeholder_alignment.py" <<'PY'
import json, sys
from pathlib import Path
root = Path(sys.argv[1])
queue = json.loads((root / '.pi/agent/package/templates/runtime/queue.json').read_text(encoding='utf-8'))
assert isinstance(queue, dict), f"expected runtime queue template object, found {type(queue).__name__}"
assert queue == {
    'version': 1,
    'paused': False,
    'activeJobId': None,
    'jobs': [],
}, f"unexpected empty runtime queue template state: {queue!r}"
print('queue-runtime-template-ok')
PY

  if "$PYTHON_BIN" "$TMP_ROOT/check_2_runtime_placeholder_alignment.py" "$REPO_ROOT" >"$out" 2>&1; then
    local detail="Queue runtime template matches the finalized empty-state object."
    record_result "$name" "PASS" "$detail"
    append_summary_row "$name" "PASS" "$detail"
    append_check_section "$name" "PASS" "$cmd" "- runtime template output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  else
    local detail="Queue runtime template does not yet match the finalized empty-state object."
    record_result "$name" "FAIL" "$detail"
    append_summary_row "$name" "FAIL" "$detail"
    append_check_section "$name" "FAIL" "$cmd" "- output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  fi
}

check_3_queue_docs_alignment() {
  local name="3. queue docs describe the finalized contract"
  local out="$TMP_ROOT/check_3_queue_docs_alignment.txt"
  local cmd="$PYTHON_BIN $TMP_ROOT/check_3_queue_docs_alignment.py"

  cat > "$TMP_ROOT/check_3_queue_docs_alignment.py" <<'PY'
import sys
from pathlib import Path
root = Path(sys.argv[1])
doc = (root / '.pi/agent/docs/queue_semantics.md').read_text(encoding='utf-8')
required_snippets = [
    'Current queue shape is a versioned top-level object:',
    '## Queue-level fields',
    '### `paused`',
    '### `activeJobId`',
    '## Who may create jobs',
    'Blocked is not the same as failed.',
    '- `qualityInput`',
    'structured `worker_to_quality` handoff',
    'structured `quality_to_validator` handoff',
    'queued `validator_worker` quality jobs',
]
missing = [snippet for snippet in required_snippets if snippet not in doc]
assert not missing, f"missing expected queue doc snippets: {missing}"
assert 'Current queue shape is a top-level array:' not in doc, 'obsolete top-level array wording still present'
print('queue-docs-ok')
PY

  if "$PYTHON_BIN" "$TMP_ROOT/check_3_queue_docs_alignment.py" "$REPO_ROOT" >"$out" 2>&1; then
    local detail="Queue docs describe the versioned runtime shape and key lifecycle semantics."
    record_result "$name" "PASS" "$detail"
    append_summary_row "$name" "PASS" "$detail"
    append_check_section "$name" "PASS" "$cmd" "- docs output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  else
    local detail="Queue docs still lag the finalized queue contract."
    record_result "$name" "FAIL" "$detail"
    append_summary_row "$name" "FAIL" "$detail"
    append_check_section "$name" "FAIL" "$cmd" "- output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  fi
}

check_4_discoverability_wiring() {
  local name="4. queue validator is discoverable in operator and repo workflows"
  local out="$TMP_ROOT/check_4_discoverability_wiring.txt"
  local cmd="$PYTHON_BIN $TMP_ROOT/check_4_discoverability_wiring.py"

  cat > "$TMP_ROOT/check_4_discoverability_wiring.py" <<'PY'
import sys
from pathlib import Path
root = Path(sys.argv[1])
operator = (root / '.pi/agent/docs/operator_workflow.md').read_text(encoding='utf-8')
readme = (root / 'README.md').read_text(encoding='utf-8')
static = (root / 'scripts/check-repo-static.sh').read_text(encoding='utf-8')
ci = (root / '.github/workflows/ci.yml').read_text(encoding='utf-8')
required = {
    'operator_workflow': './scripts/validate-queue-semantics.sh',
    'readme': 'queue-semantics validator',
    'check-repo-static': 'scripts/validate-queue-semantics.sh',
    'ci': 'Run queue-semantics validator',
}
checks = {
    'operator_workflow': required['operator_workflow'] in operator,
    'readme': required['readme'] in readme,
    'check-repo-static': required['check-repo-static'] in static,
    'ci': required['ci'] in ci,
}
missing = [name for name, ok in checks.items() if not ok]
assert not missing, f"missing discoverability/wiring in: {missing}"
print('queue-discoverability-ok')
PY

  if "$PYTHON_BIN" "$TMP_ROOT/check_4_discoverability_wiring.py" "$REPO_ROOT" >"$out" 2>&1; then
    local detail="Queue validator is wired into operator docs, static checks, README, and CI."
    record_result "$name" "PASS" "$detail"
    append_summary_row "$name" "PASS" "$detail"
    append_check_section "$name" "PASS" "$cmd" "- discoverability output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  else
    local detail="Queue validator is not yet fully wired into operator docs, static checks, README, and CI."
    record_result "$name" "FAIL" "$detail"
    append_summary_row "$name" "FAIL" "$detail"
    append_check_section "$name" "FAIL" "$cmd" "- output:\n\n\`\`\`\n$(cat "$out")\n\`\`\`"
  fi
}

write_header
check_1_queue_schema_contract
check_2_runtime_placeholder_alignment
check_3_queue_docs_alignment
check_4_discoverability_wiring

cat "$SUMMARY_TABLE_FILE" >> "$REPORT_PATH"
cat "$DETAILS_FILE" >> "$REPORT_PATH"
cat >> "$REPORT_PATH" <<EOF

## Final Decision
- Overall status: $( [[ $FAILED_CHECKS -eq 0 ]] && echo PASS || echo FAIL )
- Failed checks: $FAILED_CHECKS
- Summary JSON: $SUMMARY_JSON_PATH
EOF

write_json_summary

if [[ $FAILED_CHECKS -eq 0 ]]; then
  echo "Queue-semantics validation PASS"
  echo "Report: $REPORT_PATH"
  echo "Summary JSON: $SUMMARY_JSON_PATH"
  exit 0
else
  echo "Queue-semantics validation FAIL"
  echo "Report: $REPORT_PATH"
  echo "Summary JSON: $SUMMARY_JSON_PATH"
  exit 1
fi
