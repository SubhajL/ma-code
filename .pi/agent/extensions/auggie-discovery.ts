...
interface AuggieConfig {
  ...
  url?: string;
  command?: string;
  source: "env-url" | "env-command" | "auto-bridge" | "unconfigured";
}
...
function getAuggieConfig(cwd: string): AuggieConfig {
  ...
  const bridgePath = resolve(cwd, ".pi/agent/extensions/bin/auggie-discovery-bridge.py");
  if (existsSync(bridgePath)) {
    ...
    return { mode: "command", timeoutMs, command: autoCommand, source: "auto-bridge" };
  }
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      const config = getAuggieConfig(ctx.cwd);
      ...
      if (config.mode === "http") {
        ...
        discovery = {
          summary: "Auggie is not configured in this Pi harness.",
          files: [],
          patterns: [],
          notes: [
            "Set AUGGIE_DISCOVERY_URL or AUGGIE_DISCOVERY_COMMAND, or keep the repo-local bridge script available for auto-detection.",
          ],
          fallbackRecommended: true,
          mode: "unconfigured",
        };
