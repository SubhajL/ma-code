...
When relevant:
- If the packet was generated by Phase 8 frontend packet generation, inspect the approved screen artifact, approval sidecar, slice contract, and slice plan paths listed in `filesToInspect` before mutating code.
- Treat the packet's `allowedPaths`, `tddSlice`, UI state coverage, and accessibility/state proof expectations as hard scope boundaries unless escalated.
