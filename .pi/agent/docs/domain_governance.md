...
- Phase 8 frontend packet generation (`harness:fe-packet`) creates preview-only `frontend_worker` packets for UI-facing slices, requires explicit frontend `allowedPaths`, and carries approved screen/contract references through existing packet fields.
- Phase 9 backend packet generation (`harness:be-packet`) follows FE validation, creates preview-only `backend_worker` packets for backend-applicable slices, requires explicit backend `allowedPaths`, and carries FE evidence/contract references through existing packet fields.
- Phase 10 slice dependency decision (`harness:slice-dependencies`) compares slice summaries before any future cross-slice parallel scheduling. It blocks missing proof, shared files/contracts/schema/config/tests/fixtures, same-slice comparisons, and unknown lease/worktree conflict checks for scheduling readiness; it does not create queue jobs or change queue-runner dispatch.
