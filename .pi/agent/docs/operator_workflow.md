...
- use `bash ./scripts/validate-prompt-semantics-live.sh` when one bounded provider-backed semantic proof is needed after the local semantic validator is already green. Pass `--skip-live` in CI or any context where hitting a live provider is undesirable; the live-probe check then records a deterministic `SKIP` with a distinct detail string (`Live probe skipped by --skip-live flag (caller opted out of provider-backed validation).`) so consumers can tell from the JSON exactly why the probe did not run
...
- The normalized dry-run plan returns `writesFiles: false`, `selectedPath`, `delegatedCommand`, `status`, `blockers`, `missingArtifacts`, `hitlGates`, and `nextSafeActions`.
- `apply` invokes exactly one allowlisted helper command and then verifies helper-reported `createdFiles` against `.pi/agent/extensions/orchestrator-apply-policy.ts` write allowlists.
