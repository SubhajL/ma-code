...
export interface ClientStructuredLogEntry {
  timestamp: string;
  runtime: "web";
...
export interface ClientStructuredLogger {
  log(level: ObservabilityLogLevel, event: string, data?: Record<string, unknown>): ClientStructuredLogEntry;
  debug(event: string, data?: Record<string, unknown>): ClientStructuredLogEntry;
  info(event: string, data?: Record<string, unknown>): ClientStructuredLogEntry;
  warn(event: string, data?: Record<string, unknown>): ClientStructuredLogEntry;
  error(event: string, data?: Record<string, unknown>): ClientStructuredLogEntry;
}

export interface CreateClientLoggerOptions {
  environment?: ObservabilityEnvironment;
  now?: () => Date;
  write?: (line: string, entry: ClientStructuredLogEntry) => void;
}
...
export function createClientLogger(options: CreateClientLoggerOptions = {}): ClientStructuredLogger {
  const environment = options.environment ?? "local";
  const now = options.now ?? (() => new Date());
  const write = options.write ?? defaultWrite;

  const log = (level: ObservabilityLogLevel, event: string, data?: Record<string, unknown>): ClientStructuredLogEntry => {
    const entry: ClientStructuredLogEntry = {
...
function defaultWrite(line: string, entry: ClientStructuredLogEntry): void {
