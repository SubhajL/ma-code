...
test("apply writes an AFK run artifact and creates queue jobs only through queue helper provenance", async () => {
  ...
  const implementationCommand = queue.jobs[0]?.implementationCommand ?? "";
  assert.match(implementationCommand, /--print/);
  assert.match(implementationCommand, /--no-session/);
  assert.match(implementationCommand, /--no-extensions/);
  assert.match(implementationCommand, /--model "github-copilot\/gpt-5\.4"/);
  assert.match(implementationCommand, /--thinking "high"/);
  assert.match(implementationCommand, /\/skill:g-coding/);
  assert.doesNotMatch(implementationCommand, /\/skill:g-planning/);
  assert.doesNotMatch(implementationCommand, /&&/);
